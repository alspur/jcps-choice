...
# pull district rate
male_dist_rt <- male_dist %>% pull(pct_male)

# remove district total
male_odds <- male_odds %>%
  filter(zipcode != "High Total") %>%
  mutate(dist_comp = pct_male / male_dist_rt,
...
# there are none!
male_double <- male_odds %>%
  filter(pct_male > (2 * male_dist_rt))


male_half <- male_odds %>%
  filter(pct_male < ( male_dist_rt / 2))

male_half_list<- male_half %>%
  select(zipcode) %>%
  mutate(rate = "Half")


male_mid <- male_odds %>% 
  anti_join(male_half_list)


male_mid_list <- male_mid %>%
  select(zipcode) %>%
  mutate(rate = "About Average")

male_list <- male_half_list %>%
  bind_rows(male_mid_list) %>%
  filter(zipcode != "High Total")

male_mid_summary <- male_mid %>%
  filter(zipcode != "High Total") %>%
...
write_rds(male_geom, "data/male_geom.rda")
write_rds(male_odds, "data/male_odds.rda")
write_rds(male_dist, "data/male_dist.rda")
...
ggplot(male_odds %>%
         filter(zipcode != "High Total") %>%
         filter(Total > 10),
       aes(x = mhi_est, pct_male, size = Total)) +
  geom_point(alpha = .6)+
  ...
  geom_hline(yintercept = male_dist_rt, color = "firebrick") +
  geom_vline(xintercept = 52237, color = "firebrick") +
  labs(x = "Median Household Income",
       y = "Percent of JCPS HS Students @ Male",
       title = "Access to Male HS by Louisville Zip Code", 
       size = "JCPS HS Students\nin Zip Code",
       caption = "Vertical red line = Median Household Income for Jefferson County\nHorizontal red line = percent of all JCPS HS students attending Louisville Male HS\nDotted red line = linear relationship between zip income & enrollment")
...
ggplot(male_geom) +
  ...
  geom_point(data = profile_sch %>% filter(year == "2016-2017" & sch_id == "275047"),
             aes(x = long, y = lat),
  ...
  coord_sf(datum = NA) +
  theme_void()+
  labs(title = "Access to Louisville Male HS by Zip Code",
